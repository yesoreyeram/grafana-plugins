...
const PluginId = "yesoreyeram-duckdb-datasource"
...
func (ds *PluginHost) CheckHealth(ctx context.Context, req *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	return &backend.CheckHealthResult{
		...
		Message: "duckdb datasource just works but does nothing",
	}, nil
}
func (ds *PluginHost) QueryData(ctx context.Context, req *backend.QueryDataRequest) (*backend.QueryDataResponse, error) {
	...
	for _, q := range req.Queries {
		frame := data.NewFrame(
			q.QueryType, data.NewField("response", nil, []string{"duckdb response"}),
		).SetMeta(
			&data.FrameMeta{Notices: []data.Notice{{Text: "duckdb query works. but not fully implemented"}}},
		)
